use crate::{
    helpers::{may_deserialize, namespaces_with_key},
    traits::Borsh,
};
...
mod tests {
    ...
    #[derive(Serialize, Deserialize, BorshSerialize, BorshDeserialize, PartialEq, Debug, Clone)]
    struct Data {
        pub name: String,
        pub age: i32,
    }
    ...
    fn test_paging() -> StdResult<()> {
        ...
        let append_store: Deque<u32> = Deque::new("test");

        let page_size: u32 = 5;
        let total_items: u32 = 50;

        for i in 0..total_items {
            append_store.push_back(&mut storage, &i)?;
        }
